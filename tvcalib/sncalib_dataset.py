...
class BaseDataset(Dataset):
    def __init__(
        ...
        self.dir_images = images_dir
        self.image_files = sorted(list(self.dir_images.glob("*.jpg")))
        # if constant_cam_position > 1:
        #     self.df_match_info = (
        #         self.df_match_info.groupby(
        #             ["league", "season", "match"]).agg(list).reset_index()
        #     )
    ...
    def __getitem__(self, idx):
        ...
        candidates_meta = {}
        candidates_meta["image_id"] = self.image_files[idx].name.split(".")[0]

        # if self.constant_cam_position == 1:
        #     candidates_meta = {k: [v] for k, v in candidates_meta.items()}

        # candidates_meta["keypoints_raw"] =
        # for image_id in self.image_files:  # candidates_meta["image_id"]:
        file_annotation = self.dir_annotations / candidates_meta["image_id"]
        file_annotation = (
        ...
        for l in self.model3d.segment_names:
            if l not in keypoints_dict:
                keypoints_dict[l] = []
        candidates_meta["keypoints_raw"] = keypoints_dict
...
class FixedInputSizeDataset(BaseDataset):
    ...
    def prepare_per_sample(self, keypoints_raw: dict, image_id: str):
        ...
        for segment_type, num_segments, segment_names in [
            ("lines",
             self.model3d.line_segments.shape[1], self.model3d.line_segments_names),
            ("circles",
             self.model3d.circle_segments.shape[1], self.model3d.circle_segments_names),
        ]:
        ...
        if self.return_image:
            file_image = self.dir_images / (image_id + ".jpg")
    ...
    def __getitem__(self, idx):
        ...
        image_id = meta_dict["meta"]["image_id"]
        keypoints_raw = meta_dict["meta"]["keypoints_raw"]

        per_sample_output = self.prepare_per_sample(keypoints_raw, image_id)
...
class CamInferenceDataset(FixedInputSizeDataset):
    def __init__(
        ...
        self.df_per_sample_output_cams.set_index(
            "image_ids", drop=True, inplace=True)
